...
- **pre-compaction** → `GC_MANAGED_SESSION_HOOK=1 gc hook run
  --when-managed-session -- handoff --auto "context cycle"` (capture state
  before a managed provider compacts the conversation). The marker selects the
  managed hook; the handoff then requires a complete Gas City session identity
  and exits non-zero, naming the missing variables, when any part is absent.
