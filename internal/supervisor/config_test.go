...
func TestLoadConfigEncryptedCredentials(t *testing.T) {
	path := filepath.Join(t.TempDir(), "supervisor.toml")
	if err := os.WriteFile(path, []byte(`
[credentials]
[[credentials.encrypted]]
id = "openai-token"
path = "/var/lib/gascity/credentials/openai-token.cred"
env = "OPENAI_API_KEY"
providers = ["codex"]
`), 0o644); err != nil {
		t.Fatal(err)
	...
	if got := cfg.Credentials.Encrypted; len(got) != 1 {
		t.Fatalf("encrypted credentials = %#v, want one entry", got)
	}
	cred := cfg.Credentials.Encrypted[0]
	if cred.ID != "openai-token" || cred.Path != "/var/lib/gascity/credentials/openai-token.cred" || cred.Env != "OPENAI_API_KEY" {
		t.Fatalf("credential = %#v", cred)
	}
	if !slices.Equal(cred.Providers, []string{"codex"}) {
		t.Fatalf("credential providers = %#v, want codex", cred.Providers)
	}
}

func TestLoadConfigEncryptedCredentialsRejectsInvalidMappings(t *testing.T) {
	tests := []struct {
		name     string
		body     string
		wantText string
...
[[credentials.encrypted]]
id = "openai-token-2"
path = "/var/lib/gascity/credentials/openai-token-2.cred"
env = "OPENAI_API_KEY"
providers = ["claude"]
`,
			wantText: "duplicates",
...
providers = []
`,
			wantText: "must name at least one provider",
		},
		{
			name: "invalid id",
			body: `
...
providers = ["codex"]
`,
			wantText: "systemd credential id",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err == nil {
				t.Fatal("LoadConfig accepted invalid encrypted credential mapping")
			}
			if !strings.Contains(err.Error(), tt.wantText) {
				t.Fatalf("LoadConfig error = %v, want text %q", err, tt.wantText)
			}
		})
	}
}
