	...
	AuditBaseline: []Baseline{
		{
			...
			ResourceOwner:   "ga-80po0c.2 owns this point-in-time source census; the dc-old absorption (gct-p5v2d) added one reviewed exec.Command('sh', ...) call in the new examples/bd/dolt/cleanup_test.go, which exercises the real cleanup script end-to-end",
	...
	Debt: []Baseline{
		{
			...
			ResourceOwner:   "each process-owning test removes or replaces its source call site; the dc-old absorption (gct-p5v2d) added one reviewed exec.Command('sh', ...) call in the new examples/bd/dolt/cleanup_test.go, which exercises the real cleanup script end-to-end",
	...
	SmallDebt: []Baseline{
		{
			...
			ResourceOwner:   "non-Medium lexical owners remove or replace each process call site; the dc-old absorption (gct-p5v2d) added one reviewed exec.Command('sh', ...) call in the new examples/bd/dolt/cleanup_test.go, which exercises the real cleanup script end-to-end",
			MigrationTarget: "D1/D2/D5/D6/E6",
			Expires:         "2026-10-01",
		},
		{
