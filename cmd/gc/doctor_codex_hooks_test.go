...
func TestCodexHooksDriftCheckPassesCurrentHooks(t *testing.T) {
  ...
  "hooks": {
    ...
    "PreCompact": [{
      "hooks": [{
        "type": "command",
        "command": "export PATH=\"$HOME/go/bin:$HOME/.local/bin:$PATH\" && GC_MANAGED_SESSION_HOOK=1 gc --city %s hook run --when-managed-session -- handoff --auto --hook-format codex \"context cycle\""
      }]
    }]
  }
